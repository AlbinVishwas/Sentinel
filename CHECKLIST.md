...
> Check items off as they are completed. Last updated: May 31, 2026.
...
- [ ] Install the `google-adk` package in the backend.
- [ ] Configure Vertex AI / Google Cloud credentials for local development.
- [ ] Initialize the Gemini 3.5 Flash model within the FastAPI app.
- [ ] Stand up the GitLab MCP server locally.
- [ ] Connect the ADK orchestrator to the GitLab MCP server.
- [ ] Map the GitLab MCP "Read" tools (e.g. `get_issue`, `search_repository`, `read_file`).
- [ ] Smoke-test a read-only round trip (prompt → tool call → response).
...
- [ ] Scope the GitLab PAT to `read_api` + `write_repository` only.
